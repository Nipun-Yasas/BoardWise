    ...
    try {
        ...
        const boardingsWithStats = await Promise.all(
            boardings.map(async (boarding) => {
                ...
                const availableRooms = rooms.filter(r => r.isAvailable !== false && (r.tenants?.length || 0) < r.capacity);

                const minPrice = availableRooms.length > 0
                    ? Math.min(...availableRooms.map((r) => r.price))
                    : rooms.length > 0 ? Math.min(...rooms.map((r) => r.price)) : 0; // Fallback to any room if none available

                // Capacity = Total available slots
                const totalAvailableSlots = rooms.reduce((acc, room) => {
                    const occupied = room.tenants?.length || 0;
                    const capacity = room.capacity || 0;
                    const available = room.isAvailable !== false ? Math.max(0, capacity - occupied) : 0;
                ...
                return {
                    ...
                    title: boarding.name,
                    university: boarding.city || "Unknown", // Fallback to city as university
                    distance: 0, // Placeholder as we don't have location data
                    rental: minPrice,
                    persons: totalAvailableSlots, // Use AVAILABLE slots instead of total capacity
                    imageUrl: boarding.mainImage || "https://images.unsplash.com/photo-1555854877-bab0e564b8d5?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80", // Default image if missing
